...
const youtubeNotFound = (
  <div className="p-3 text-2xl">No YouTube stream found :(</div>
);
...
function Section({ title, className, onHide, children }: SectionProps) {
  return (
    ...
    >
      ...
      <div className="grow flex items-center place-content-center">
        {children}
      </div>
    </div>
  );
...
export default function LivePortal({
  ...
  const [showYoutubeVideo, setShowYoutubeVideo] = useState(!!youtubeVideoId);
  const [showTwitchVideo, setShowTwitchVideo] = useState(true);

  const [showTwitchChat, setShowTwitchChat] = useState(true);
  const [showYoutubeChat, setShowYoutubeChat] = useState(!!youtubeVideoId);
        ...
        <div className="grow flex flex-col gap-3">
          {showYoutubeVideo && (
            <Section
              title={
                <>
                  ...
                  {!youtubeVideoId && <div> YouTube Video </div>}
                </>
              }
            ...
            >
              ...
              {!youtubeVideoId && youtubeNotFound}
            </Section>
          )}
